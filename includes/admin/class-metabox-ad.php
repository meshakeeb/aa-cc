...
class Metabox_Ad implements Integration_Interface {
	...
	public function remove_from_ad_group( $post_id ) {
		...
		if ( ! isset( $_POST['post_type'] ) || Constants::POST_TYPE_AD !== $_POST['post_type'] ) {
			return;
		}

		if ( isset( $_POST['advanced_ad']['output']['group_id'] ) ) {
			$group_id = absint( $_POST['advanced_ad']['output']['group_id'] );
			if ( isset( $_POST['tax_input']['advanced_ads_groups'] ) ) {
				$key = array_search( $group_id, $_POST['tax_input']['advanced_ads_groups'] ); // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
				if ( false !== $key ) {
					wp_remove_object_terms( $post_id, $group_id, Constants::TAXONOMY_GROUP );
