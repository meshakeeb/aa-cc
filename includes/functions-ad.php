 ...
 *
 * @return string The ad content.
 */
function get_the_ad( $ad_id = 0, $new_type = '', $args = [] ): string {
	// Early bail!!
	if ( isset( $args['override'] ) ) {
		return $args['override'];
	}

	$ad = is_a( $ad_id, Ad::class ) ? $ad_id : wp_advads_get_ad( $ad_id, $new_type );
	if ( ! $ad || 0 === $ad->get_id() ) {
		return '';
	}
 ...
 * @return void
 */
function wp_advads_delete_ad( &$ad, $args = [] ): void {
	wp_advads_get_ad_repository()->delete( $ad, $args = [] );
}
