...
class Plain implements Ad_Type {
	...
	private function render_php_allow( $ad ) {
		...
		if ( 1 !== $allow_php ) {
			$allow_php = absint( $ad->is_php_allowed() );
		}
		?>
		...
		<div>
			<input id="advads-parameters-php" type="checkbox" name="advanced_ad[output][allow_php]" value="1" <?php checked( 1, $allow_php ); ?> onChange="Advanced_Ads_Admin.check_ad_source();" <?php disabled( ! Conditional::is_php_allowed() ); ?>/>
	...
	private function error_unfiltered_html( $ad ): void {
		...
		<p class="advads-notice-inline advads-error">
			...
			if ( $author_id === $current_user_id ) {
				...
				if ( current_user_can( 'unfiltered_html' ) && Conditional::has_user_role_on_site() ) {
