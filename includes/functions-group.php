 ...
 *
 * @return string The group content.
 */
function get_the_group( $group_id = 0, $new_type = '', $args = [] ): string {
	// Early bail!!
	if ( isset( $args['override'] ) ) {
		return $args['override'];
	}

	$group = is_a( $group_id, Group::class ) ? $group_id : wp_advads_get_group( $group_id, $new_type );
 ...
 * @return void
 */
function wp_advads_delete_group( &$group, $args = [] ): void {
	wp_advads_get_group_repository()->delete( $group, $args = [] );
}
