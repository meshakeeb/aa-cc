...
abstract class Admin_List_Table implements Integration_Interface {
	...
	public function enqueue_scripts(): void {
		...
		if ( Params::get( 's' ) ) {
			wp_add_inline_style(
				Assets_Registry::prefix_it( 'common' ),
				"
		...
		if ( Conditional::has_filter_or_search() && 0 === $wp_query->found_posts ) {
			wp_add_inline_style( Assets_Registry::prefix_it( 'common' ), ".post-type-{$this->list_table_type} .search-box { display: block; margin-top: 10px; }" );
			return;
		}
		...
		if ( get_current_screen()->get_option( 'show-filters' ) || Conditional::has_filter_or_search() ) {
			wp_add_inline_style( Assets_Registry::prefix_it( 'common' ), ".post-type-{$this->list_table_type} .search-box { display: block !important; }" );
			if ( isset( $wp_query->found_posts ) && $wp_query->found_posts > 0 ) {
				wp_add_inline_style( Assets_Registry::prefix_it( 'admin' ), ".post-type-{$this->list_table_type} .tablenav.top .alignleft.actions:not(.bulkactions) { display: block; }" );
			}
		}
	}
