...
class Advanced_Ads_Import {
			...
			foreach ( $decoded['ads'] as $k => $ad ) {
				...
				if ( ! empty( $ad['groups'] ) && is_array( $ad['groups'] ) ) {
					...
					foreach ( $ad['groups'] as $_group ) {
						...
						$advads_ad_weights[ $group_id ][ $post_id ] = absint( $_group['weight'] ?? Constants::GROUP_AD_DEFAULT_WEIGHT );
					}
					...
					$tt_ids = wp_set_post_terms( $post_id, $groups_to_set, Constants::TAXONOMY_GROUP  );
				}
			}
		}
	}
	...
	private function create_group_term( $_group ) {
		...
		if ( term_exists( $slug, Constants::TAXONOMY_GROUP ) ) {
			$count = 1;
			while ( term_exists( $slug . '_' . $count, Constants::TAXONOMY_GROUP ) ) {
				++$count;
			}
			$slug = $slug . '_' . $count;
		}

		$t = wp_insert_term( $_group['name'], Constants::TAXONOMY_GROUP, [ 'slug' => $slug] );
		...
		if ( ! is_wp_error( $t ) ) {
			...
			$this->messages[] = [ 'error', sprintf( __( 'Failed to import taxonomy: <em>%1$s</em>, term: <em>%2$s</em>', 'advanced-ads' ), esc_html( Constants::TAXONOMY_GROUP ), esc_html( $_group['name'] ) ) ];
			if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
	...
	private function import_placements( &$decoded ) {
		if ( isset( $decoded['placements'] ) && is_array( $decoded['placements'] ) ) {
			...
			$existing_placements = $updated_placements = wp_advads_get_placements();
							...
							case Constants::ENTITY_AD:
								...
								if ( $use_existing ) {
									...
									if ( ! empty( $placement['item'] ) ) {
										...
										if ( ! empty( $_item_existing[1] ) && $_item_existing[0] === Constants::ENTITY_GROUP ) {
											$advads_ad_weights = get_option( 'advads-ad-weights', [] );
