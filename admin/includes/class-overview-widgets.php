...
class Advanced_Ads_Overview_Widgets_Callbacks {
	...
	public static function render_next_steps() {
		...
		$recent_ads = wp_advads_ad_query()->posts;
		if ( count( $recent_ads ) === 0 ) :
			echo '<p><a class="button button-primary" href="' . esc_url( admin_url( 'post-new.php?post_type=' . Constants::POST_TYPE_AD ) ) .
		...
		if ( $can_subscribe ) {
			...
			// link to manage ads.
			echo '<p><a class="button button-secondary" href="' . esc_url( admin_url( 'edit.php?post_type=' . Constants::POST_TYPE_AD ) ) .
	...
	public static function render_addons( $hide_activated = false ) {
		...
		$caching_used = Conditional::has_cache_plugins();
		...
		// STICKY.
		if ( isset( $installed_plugins['advanced-ads-sticky-ads/sticky-ads.php'] ) && ! function_exists( 'wp_advads_stickyads' ) ) { // is installed, but not active.
			...
			++$installed_pro_plugins;
		} elseif ( function_exists( 'wp_advads_stickyads' ) ) {
