...
jQuery(document).ready(function () {
	...
	jQuery(document).on(
		'click',
		'.adsvads-ad-health-notices-show-hidden',
		function () {
	...
	jQuery('.advads-help').on('mouseenter', function (event) {
		...
		while (
			tooltipRect.bottom >
			Math.min(topParentRect.bottom, jQuery(window).height()) - 20
		) {
			offsets.top -= 10;
			tooltip.style.top = offsets.top + 'px';
			tooltipRect = tooltip.getBoundingClientRect();
...
function advads_ad_health_reload_show_link() {
	...
	const show_link = jQuery('.adsvads-ad-health-notices-show-hidden');
